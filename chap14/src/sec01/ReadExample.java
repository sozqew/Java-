...
public class ReadExample {
	public static void main(String[] args) throws Exception {
		...
		*/
		/*
		int data1 = is.read();	// test1.db 안의 1바이트 읽어 리턴 만약에 못 읽으면 -1을 리턴
		System.out.println("1byte -> " + data1);	// 10
		int data2 = is.read();	// test1.db 안의 다음 1바이트 읽어 리턴 만약에 못 읽으면 -1을 리턴
		...
		*/
		...
		int readByteNum = is.read(buffer);	// buffer 크기만큼 test2.db에 있는 데이터를 가져옴
		for(byte b1 : buffer) {
